...
namespace Microsoft.AspNetCore.Server.Kestrel.Http
{
    public class Connection : ConnectionContext, IConnectionControl
    {
        ...
        public void Start()
        {
            NetworkLogger.ConnectionStart(Log, ConnectionId);
            ...
            else
            {
                ...
                try
                {
                    ServerInformation.ConnectionFilter.OnConnectionAsync(_filterContext).ContinueWith((task, state) =>
                    {
                        ...
                        if (task.IsFaulted)
                        {
                            connection.Log.Error(0, "ConnectionFilter.OnConnection", task.Exception);
                            connection.ConnectionControl.End(ProduceEndType.SocketDisconnect);
                        }
                        else if (task.IsCanceled)
                        {
                            connection.Log.Error(0, "ConnectionFilter.OnConnection Canceled", null);
                            connection.ConnectionControl.End(ProduceEndType.SocketDisconnect);
                        }
                        else
                        {
                ...
                {
                    Log.Error(0, "ConnectionFilter.OnConnection", ex);
        ...
        private void OnRead(UvStreamHandle handle, int status)
        {
            ...
            if (normalRead)
            {
                NetworkLogger.ConnectionRead(Log, ConnectionId, readCount);
            }
            else
            {
                _socket.ReadStop();
                NetworkLogger.ConnectionReadFin(Log, ConnectionId);
            }
        ...
        {
            NetworkLogger.ConnectionPause(Log, ConnectionId);
            _socket.ReadStop();
        }

        void IConnectionControl.Resume()
        {
            NetworkLogger.ConnectionResume(Log, ConnectionId);
            _socket.ReadStart(_allocCallback, _readCallback, this);
        }
            ...
            lock (_stateLock)
            {
                switch (endType)
                {
                    case ProduceEndType.ConnectionKeepAlive:
                        ...
                        NetworkLogger.ConnectionKeepAlive(Log, ConnectionId);
                        break;
                    ...
                    case ProduceEndType.SocketDisconnect:
                        ...
                        NetworkLogger.ConnectionDisconnect(Log, ConnectionId);
