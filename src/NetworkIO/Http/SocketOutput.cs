    ...
    public class SocketOutput : ISocketOutput
    {
            ...
            bool isSync = false)
        {
            ...
            lock (_contextLock)
            {
                if (_socket.IsClosed)
                {
                    NetworkLogger.ConnectionDisconnectedWrite(_log, _connectionId, buffer.Count, _lastWriteError);
        ...
        private void CancellationTriggered()
        {
            lock (_contextLock)
            {
                if (!_cancelled)
                {
                    ...
                    NetworkLogger.ConnectionError(_log, _connectionId, new TaskCanceledException("Write operation canceled. Aborting connection."));
                }
            }
        }
        ...
        private void OnWriteCompleted(WriteContext writeContext)
        {
            ...
            if (error == null)
            {
                ...
                NetworkLogger.ConnectionWriteCallback(_log, _connectionId, status);
            }
            else
            {
                CompleteAllWrites();
                NetworkLogger.ConnectionError(_log, _connectionId, error);
            }
        }
        ...
        private class WriteContext
        {
            ...
            public void DoWriteIfNeeded()
            {
                ...
                {
                    _writeReq = new UvWriteReq() { Log = Self._log };
            ...
            public void DoShutdownIfNeeded()
            {
                ...
                NetworkLogger.ConnectionWriteFin(Self._log, Self._connectionId);

                var shutdownReq = new UvShutdownReq() { Log = Self._log };
                ...
                shutdownReq.Shutdown(Self._socket, (_shutdownReq, status, state) =>
                {
                    ...
                    NetworkLogger.ConnectionWroteFin(_this.Self._log, _this.Self._connectionId, status);
            ...
            public void DoDisconnectIfNeeded()
            {
                ...
                NetworkLogger.ConnectionStop(Self._log, Self._connectionId);
                CompleteWithContextLock();
            }
            ...
            public void CompleteOnThreadPool()
            {
                lock (Self._contextLock)
                {
                    ...
                    {
                        Self._log.Error(0, "SocketOutput.OnWriteCompleted", ex);
                    }
                }
            }
